...
const AddCourses = () => {
  ...
  // const handleLecture = (action, chapterId, lectureIndex) => {
  //   console.log('handleLecture called:', action, chapterId, lectureIndex);
  //   if (action === 'add') {
  //     setCurrentChaptersId(chapterId);
  //     setShowPopup(true);
  //   } else if (action === 'remove') {
  ...
  //       chapters.map((chapter) => {
  //         if (chapter.chapterId === chapterId) {
  //           console.log('Removing lecture at index:', lectureIndex);
  //           chapter.chapterContent.splice(lectureIndex, 1);
  //         }
  //         return chapter;
  //       })
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md w-full text-gray-500">
        ...
        <div className='flex flex-col gap-1'>
          <p>Discount %</p>
          <input type="number" onChange={e => setDiscount(e.target.value)} value={discount} placeholder='0' min={0} max={100} className='outline-none md:py-2.5
            ...
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
              <div className='flex justify-between items-center p-4 border-b'>
                <div className='flex items-center'>
                  ...
                  src={assets.dropdown_icon} width={14} className={`mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`} />
              ...
              {!chapter.collapsed && (
                <div className='p-4'>
                  ...
                  <div className='inline-flex bg-gray-200 p-2 rounded cursor-pointer mt-2' onClick={() => handleLecture('add', chapter.chapterId)}> + Add Lecture</div>
                </div>
              )}
            </div>
          ))}
          <div className='flex justify-center items-center bg-blue-200 p-2 rounded-lg cursor-pointer' onClick={() => handleChapter('add')}> + Add Chapter</div>
              ...
              <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                ...
                <div className='mb-2'>
                  ...
                  <input type="text" className='mt-1 block w-full border rounded py-1 px-2'
                    value={lectureDetails.lectureTitle} onChange={(e) => setLecturesDetails({ ...lectureDetails, lectureTitle: e.target.value })} />
                </div>

                <div className='mb-2'>
                  <p>Duration (minutes)</p>
                  <input type="number" className='mt-1 block w-full border rounded py-1 px-2'
                    value={lectureDetails.lectureDuration} onChange={(e) => setLecturesDetails({ ...lectureDetails, lectureDuration: e.target.value })} />
                </div>

                <div className='mb-2'>
                  <p>Lecture URL</p>
                  <input type="text" className='mt-1 block w-full border rounded py-1 px-2'
                    value={lectureDetails.lectureUrl} onChange={(e) => setLecturesDetails({ ...lectureDetails, lectureUrl: e.target.value })} />
                </div>

                <div className='flex gap-2 my-4'>
                  <p>Is Preview Free?</p>
                  <input type="checkbox" className='mt-1 scale-125'
                    checked={lectureDetails.isPreviewFree} onChange={(e) => setLecturesDetails({ ...lectureDetails, isPreviewFree: e.target.checked })} />
                </div>

                <button type='button' className='w-full bg-blue-400 text-white px-4 py-2 rounded' onClick={addLecture}>Add</button>

                <img src={assets.cross_icon} className="absolute top-4 right-4 w-4 cursor-pointer" onClick={() => setShowPopup(false)} />
              </div>
            </div>
          )
    ...
    // <div>AddCourseknkknknks</div>
  )
}

export default AddCourses
// 6.51.00     =  frontend finished 